...
preserve
	...
	*end
	...
	egen shockFreq2yr = mean(hh_s8q06), by(hh_s8q0a)

	* Investigate collapsed results in R using d3heatmap package
	collapse (mean) pctShock totShock rankShock1 rankShock2 income_wf1-shockFreq2yr, by(hh_s8q0a)
	...
	* Replace 1, 2, 3 with increase, decrease, no change
	...
	export delimited "$pathexport/shock_heatmap2016.csv", replace
...
* Create standard categories for shocks using two types of groupings
	g byte ag 		= inlist(hh_s8q00, 108, 111, 112) &  inlist(hh_s8q02, 1, 2, 3)
	...
	g byte health 	= inlist(hh_s8q00, 101, 102, 200, 201) &  inlist(hh_s8q02, 1, 2, 3)
...
/* World Bank classifications
	...
	g byte healthShk = inlist(hh_s8q00, 101, 102, 200, 201) &  inlist(hh_s8q02, 1, 2)
...
/* Coping Mechanisms - What are good v. bad coping strategies? From (Heltberg et al., 2013)
	...
	http://documents.worldbank.org/curated/en/333551467987849378/pdf/100254-JRN-Box393221B-PUBLIC-Embargoed-until-2016-09-06.pdf
...
/* Other ways of classifying coping:
	Dissaved
	Reduced consumption
	Sold assets
	Borrowed
	...
	* How are most commong shocks coped w?
	tab hh_s8q04_a hh_s8q00 if inlist(hh_s8q00, 110) & rptShock == 1  , mi
	tab hh_s8q04_a hh_s8q00 if inlist(hh_s8q00, 104) & rptShock == 1  , mi
	tab hh_s8q04_a hh_s8q00 if inlist(hh_s8q00, 102) & rptShock == 1  , mi
...
* Load
use "$wave2\sect8_hh_w2.dta", clear
...
* How are most commong shocks coped w?
tab hh_s8q04_b hh_s8q00 if inlist(hh_s8q00, 110) & rptShock == 1  , mi
tab hh_s8q04_b hh_s8q00 if inlist(hh_s8q00, 104) & rptShock == 1  , mi
tab hh_s8q04_b hh_s8q00 if inlist(hh_s8q00, 102) & rptShock == 1  , mi
...
egen illShkwf = rsum(income_wf2 assets_wf2 foodprod_wf2 foodstock_wf2 foodpurch_wf2) if hh_s8q00 == 102
recode hh_s8q01 (2 = 0)
egen totShocks = total(hh_s8q01), by(household_id2)
...
recode region (2 5 6 12 13 15 = 20)
labmm HH_SAQ01 20 "Other regions"
la val region HH_SAQ01

* Collapse data to househld level and merge back with GIS info
ds (hh_s8* income assets foodprod foodstock foodpurch *_wf1 *_wf2 *_wf3 household_id ea_id), not
keep `r(varlist)'
...
* Copy variable labels to reapply after collapse
include "$pathdo/copylabels.do"
ds(household_id2 ea_id hid region zone woreda town subcity kebele EA rural), not
#delimit ;
	collapse (max) `r(varlist)',
	by(household_id2 ea_id2 hid region zone woreda town subcity kebele EA rural) fast; 
#delimit cr
* Reapply variable lables & value labels
include "$pathdo/attachlabels.do"
...
collapse pw saq01, by(household_id2 ea_id2)
save "$pathout/hhweights2014.dta", replace
restore

* Merge data together with sampling weights and create geovars
merge 1:1 household_id2 using "$pathout/hhweights2014.dta", gen(hhwgt_merge)
egen geoGroup = group(lat_dd_mod lon_dd_mod)
